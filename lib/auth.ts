...
// //   return user;
// // };
// lib/auth.ts (النسخة النهائية والنظيفة)

import { create } from "zustand";
import { User } from "./session";

// 1. أضفنا "token" إلى الواجهة
interface AuthState {
  user: User | null;
  token: string | null; // <-- الإضافة هنا
  isLoading: boolean;
  // 2. تعديل دالة التهيئة لتستقبل التوكن أيضاً
...
export const useAuthStore = create<AuthState>((set, get) => ({
  user: null,
  token: null, // <-- القيمة الافتراضية
  isLoading: true,

  // 3. تحديث الدالة لتقوم بتخزين المستخدم والتوكن
  initialize: (user, token) => {
    set({ user: user, token: token, isLoading: false });
  },

  // 4. تحديث دالة تسجيل الخروج لمسح التوكن أيضاً
  clearUser: () => {
