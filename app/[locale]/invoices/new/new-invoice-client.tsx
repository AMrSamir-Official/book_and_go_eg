...
export function NewInvoicePageClient({
  ...
  const onSubmit = (data: InvoiceFormData) => {
    ...
    const mainInvoiceEGP = convertToEGP(
      ...
      data.totalInvoiceCurrency,
      data.totalInvoiceExchangeRate
    );
    const extraIncomingTotal = (data.extraIncoming || []).reduce(
      (sum, item) =>
        sum + convertToEGP(item.amount, item.currency, item.exchangeRate),
      0
    );
    const grandTotalIncomeEGP = mainInvoiceEGP + extraIncomingTotal;

    const accommodationTotal = (data.accommodation || []).reduce(
      (sum, item) =>
        sum + convertToEGP(item.totalAmount, item.currency, item.exchangeRate),
      0
    );
    const domesticFlightsTotal = (data.domesticFlights || []).reduce(
      (sum, item) =>
        sum + convertToEGP(item.cost, item.currency, item.exchangeRate),
      0
    );
    const entranceTicketsTotal = (data.entranceTickets || []).reduce(
      (sum, item) =>
        sum + convertToEGP(item.total, item.currency, item.exchangeRate),
      0
    );
    const guideTotal = (data.guide || []).reduce(
      (sum, item) =>
        sum + convertToEGP(item.cost, item.currency, item.exchangeRate),
      0
    );
    const transportationTotal = (data.transportation || []).reduce(
      (sum, item) => {
        const mainAmountEGP = convertToEGP(
          item.amount,
          item.currency,
          ...
          <div className="flex justify-end space-x-4 no-print">
            ...
            <Button type="submit" disabled={isPending}>
              {isPending ? (
                ...
                <>
                  <Save className="mr-2 h-4 w-4" /> Create Accounting
                </>
              )}
            </Button>
          </div>
